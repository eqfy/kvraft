...
type CCoordGetLeaderServerArg struct {
	ClientInfo
	Token tracing.TracingToken
}
type CCoordGetLeaderServerReply struct {
	ServerId     uint8
	ServerIpPort string
	Token        tracing.TracingToken
}
...
func (d *KVS) Start(localTracer *tracing.Tracer, clientId string, coordIPPort string, localCoordIPPort string, localLeaderServerIPPort string, chCapacity int) (NotifyChannel, error) {
	var err error
	...
	var leaderServerReq CCoordGetLeaderServerArg = CCoordGetLeaderServerArg{ClientInfo{clientId, d.coordListenerIPPort}, d.ktrace.GenerateToken()}
	var leaderServerRes CCoordGetLeaderServerReply
	lsReqErr := coordClient.Call("CCoord.GetLeaderServer", leaderServerReq, &leaderServerRes)
	if err = checkCriticalErr(lsReqErr, "an error requesting leader server from coord: "); err != nil {
		return nil, err
	}
	d.ktrace = localTracer.ReceiveToken(leaderServerRes.Token)
	d.ktrace.RecordAction(LeaderResRecvd{clientId, leaderServerRes.ServerId})
	d.leaderServerIPPort = leaderServerRes.ServerIpPort

	/*Connect to Leader*/
	lsLAddr, lsLAddrErr := net.ResolveTCPAddr("tcp", localLeaderServerIPPort)
	if err = checkCriticalErr(lsLAddrErr, "an error resolving addr for localLeaderServerIPPort: "); err != nil {
		return nil, err
	}
	d.lsLocalTCPAddr = lsLAddr
	lsAddr, lsAddrErr := net.ResolveTCPAddr("tcp", d.leaderServerIPPort)
	if err = checkCriticalErr(lsAddrErr, "an error resolving addr for leaderServerIPPort: "); err != nil {
		return nil, err
	}
	lsTCPConn, hsTCPConnErr := net.DialTCP("tcp", lsLAddr, lsAddr)
	if err = checkCriticalErr(hsTCPConnErr, "an error dialing tcp from kvslib to leader server: "); err != nil {
		return nil, err
	}
	lsTCPConn.SetLinger(0)
	lsClient := rpc.NewClient(lsTCPConn)
	d.hsClientRPC = lsClient
...
func (d *KVS) handleFailure() {
	...
	for {
		...
		if serverFailure.ServerPosition == Leader {
			d.leaderServerLock.Lock()
			/* Close old client connection to HS */
			d.leaderServerIPPort = serverFailure.NewServerIpPort
			...
			var leaderServerReq CCoordGetLeaderServerArg = CCoordGetLeaderServerArg{ClientInfo{d.clientId, d.coordListenerIPPort}, d.ktrace.GenerateToken()}
			var leaderServerRes CCoordGetLeaderServerReply
			hsReqErr := d.coordClientRPC.Call("CCoord.GetLeaderServer", leaderServerReq, &leaderServerRes)
			if hsReqErr != nil {
				checkWarning(hsReqErr, "error requesting leader server from coord: ")
				continue
			}
			d.ktrace = d.kTracer.ReceiveToken(leaderServerRes.Token)
			d.ktrace.RecordAction(LeaderResRecvd{d.clientId, leaderServerRes.ServerId})
			d.leaderServerIPPort = leaderServerRes.ServerIpPort

			/* Start new client connection to HS */
			newHSAddr, newHSAddrErr := net.ResolveTCPAddr("tcp", d.leaderServerIPPort)
			checkWarning(newHSAddrErr, "error resolving newHSAddr upon receiving failed HS msg in handlGet:")
			newHSClientTCP, newHSClientTCPErr := net.DialTCP("tcp", d.lsLocalTCPAddr, newHSAddr)
...
func (d *KVS) sender() {
	...
	for {
		...
		if req.kind == "PUT" {
			...
			// 	if err != nil { // will now wait for leader server to have finished reconfiguring
...
func (cnl *CoordListener) ChangeLeaderServer(newServerIPPort string, isDone *bool) error {
